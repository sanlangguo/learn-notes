...
<p>下面是一个示例：</p>
<p>```ts</p>
<p>type MyUnion = 'a' | 'b' | 'c' | 'd';</p>
<p>type ExcludedUnion = Exclude&lt;MyUnion, 'b' | 'd'&gt;; // ExcludedUnion 的类型为 'a' | 'c'</p>
<p>const D:ExcludedUnion = 'a'</p>
<p>console.log(D)</p>
<p>```</p>
<p>在上述示例中，我们定义了一个名为 MyUnion 的联合类型，它包含了 'a'、'b'、'c' 和 'd' 这四个字符串字面量类型。然后，我们使用 TS.Exclude 工具将 'b' 和 'd' 从 MyUnion 联合类型中排除出去，得到一个新的类型 ExcludedUnion。</p>
...
<p>address: string;</p>
<p>}</p>
<p>// NewPerson 的类型为 { name: string; }</p>
<p>type NewPerson = Omit&lt;Person, 'age' | 'address'&gt;;</p>
<p>const a:NewPerson = {</p>
<p>name: 'john'</p>
<p>}</p>
<p>console.log(a)</p>
<p>```</p>
<p>在上述示例中，我们定义了一个名为 Person 的接口类型，它包含了 name、age 和 address 三个属性。然后，我们使用 TS.Omit 工具从 Person 类型中排除了 'age' 和 'address' 这两个属性，得到一个新的类型 NewPerson。</p>
